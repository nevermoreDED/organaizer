...
import {
  ...
  getDrivers, getAllDrivers, addDriver, deleteDriver, updateDriver,
...
export default function Logistics({ userId, currentUser }) {
  ...
  const loadDrivers = async () => {
    ...
    const actives = [...new Set(sanitized.map(d => ((d.active !== undefined ? d.active : true) ? 'Активен' : 'Не активен')).filter(Boolean))];
  ...
  const filteredDrivers = drivers.filter(d => {
    ...
    if (filterActive && ((filterActive === 'Активен' && !active) || (filterActive === 'Не активен' && active))) return false;
